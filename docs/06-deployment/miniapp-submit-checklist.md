...
- 当前收款边界：微信支付 JSAPI 已接入；提交审核前必须完成商户平台 AppID 绑定和 0.01 元真机验收
...
```text
本小程序为农村土猪认养和养殖过程展示平台。用户可查看猪只信息、养殖记录、直播/视频、发起认养登记和拼猪分享。当前版本已接入微信支付 JSAPI，认养订单由用户在小程序内完成微信支付后生效。
```

## 6. 当前未完成但不阻塞代码包

- 真实微信支付：代码已接入；仍需商户平台绑定当前小程序 AppID，并完成 0.01 元真机验收
